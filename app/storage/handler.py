...
class AzureStorage(BaseStorage):
    ...
    def _blob_service(self, custom_domain=None, connection_string=None) -> ContainerClient:
    ...
    @property
    def service(self) -> ContainerClient:
    ...
    @property
    def custom_service(self) -> ContainerClient:
    ...
    def exists(self, name):
        return self.service.get_blob_client(self._get_valid_path(name)).exists()

    def delete(self, name):
        try:
            self.service.delete_blob(
                blob=self._get_valid_path(name),
                timeout=self.timeout)
        except AzureMissingResourceHttpError:
            pass

    def size(self, name):
        properties = self.service.get_blob_client(name).get_blob_properties(
    ...
    def url(self, name, expire=None):
        ...
        if expire:
            sas_perms = BlobSasPermissions(read=True)
            sas_token = generate_blob_sas(
                account_name=self.account_name,
                account_key=self.account_key,
                container_name=self.container_name,
                blob_name=name,
                permission=sas_perms,
                expiry=self._expire_at(expire)
            )
            # self.custom_service.get_blob_client(name)
